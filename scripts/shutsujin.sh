#!/bin/bash
# ============================================================
# wezterm-shogun 出陣スクリプト (bash 版)
# ============================================================
# 使用方法:
#   ./scripts/shutsujin.sh           # 全エージェント起動
#   ./scripts/shutsujin.sh -s        # セットアップのみ（Claude起動なし）
#   ./scripts/shutsujin.sh -h        # ヘルプ表示
# ============================================================

set -e

# スクリプトのディレクトリを取得
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
cd "$PROJECT_DIR"

# 共通関数読み込み
source "$SCRIPT_DIR/../lib/common.sh"
source "$SCRIPT_DIR/../lib/pane_manager.sh"

# ============================================================
# オプション解析
# ============================================================
SETUP_ONLY=false

while [[ $# -gt 0 ]]; do
  case $1 in
  -s | --setup-only)
    SETUP_ONLY=true
    shift
    ;;
  -h | --help)
    show_help
    exit 0
    ;;
  *)
    echo "不明なオプション: $1"
    exit 1
    ;;
  esac
done

# ============================================================
# バナー表示
# ============================================================
show_battle_cry

# ============================================================
# STEP 1: 既存ペインのクリーンアップ
# ============================================================
log_info "🧹 既存の陣を撤収中..."
cleanup_existing_panes

# ============================================================
# STEP 2: キューファイルリセット
# ============================================================
log_info "📜 前回の軍議記録を破棄中..."
reset_queue_files

# ============================================================
# STEP 3: ダッシュボード初期化
# ============================================================
log_info "📊 戦況報告板を初期化中..."
init_dashboard

# ============================================================
# STEP 4: 将軍ワークスペース作成
# ============================================================
log_war "👑 将軍の本陣を構築中..."
SHOGUN_PANE=$(wezterm cli spawn --workspace shogun --cwd "$PROJECT_DIR")
log_success "  └─ 将軍 Pane ID: $SHOGUN_PANE"

# ============================================================
# STEP 5: 家老・足軽ワークスペース作成 (3x3 グリッド)
# ============================================================
log_war "⚔️ 家老・足軽の陣を構築中..."
create_multiagent_grid

# ペインIDを保存
save_pane_ids

log_success "✅ ペイン構築完了"

# ============================================================
# STEP 6: Claude Code 起動
# ============================================================
if [ "$SETUP_ONLY" = false ]; then
  log_war "👑 全軍に Claude Code を召喚中..."
  launch_claude_code_all

  log_success "✅ 全軍 Claude Code 起動完了"

  # ============================================================
  # STEP 7: 指示書読み込み
  # ============================================================
  log_info "📜 各エージェントに指示書を伝達中..."
  sleep 15 # Claude Code 起動待機

  send_instructions_to_all

  log_success "✅ 全軍に指示書伝達完了"
fi

# ============================================================
# STEP 8: 完了メッセージ
# ============================================================
show_completion_message "$SETUP_ONLY"
